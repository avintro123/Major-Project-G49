...
import { ArrowRightLeft, Trash2, FileText, ArrowRight, AlertCircle, Sparkles, ArrowLeft } from "lucide-react";
...
export default function DebateInputText() {
  ...
  const validate = () => {
    ...
    if (countWords(participantA) > MAX_WORDS) newErrors.participantA = `Max ${MAX_WORDS} words`;
    if (countWords(participantB) > MAX_WORDS) newErrors.participantB = `Max ${MAX_WORDS} words`;
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-2">Enter the topic and arguments for both sides.</p>
          </div>
          <div className="flex gap-3">
            ...
            <Button variant="ghost" size="sm" onClick={handleClear} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 gap-2">
        ...
        <Card className="mb-8 border-neon-blue/20 shadow-lg shadow-neon-blue/5">
          <CardContent className="pt-6">
            <label className="block text-sm font-medium mb-2 text-gray-300">Debate Topic</label>
            <Input
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              placeholder="e.g., Is AI beneficial for society?"
              className={cn("text-lg h-14 bg-black/40", errors.topic && "border-red-500 focus-visible:ring-red-500")}
            />
            {errors.topic && (
        ...
        <div className="flex justify-end">
          <Button size="lg" onClick={handleSubmit} className="w-full md:w-auto gap-2 shadow-lg shadow-neon-blue/20">
    ...
    <Card className={cn(
      ...
      <CardHeader className="pb-4 border-b border-white/5">
        <CardTitle className="text-lg flex justify-between items-center">
          <span className={cn(color === "blue" ? "text-neon-blue" : "text-neon-pink")}>{label}</span>
          <span className={cn(
